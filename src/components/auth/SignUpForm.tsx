...
import { isGoogleOAuthEnabled, isDiscordOAuthEnabled, isOAuthEnabled } from '@/lib/config/oauth';
...
export const SignUpForm: React.FC<SignUpFormProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result?.error) {
        setErrors({ general: 'Registration successful, but auto-login failed. Please sign in manually.' });
      } else if (result?.ok) {
    ...
    <Card className={className}>
      ...
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* Social Sign Up - Only show if OAuth is enabled */}
          {isOAuthEnabled() && (
            <>
              ...
              <div className={`grid ${isGoogleOAuthEnabled() && isDiscordOAuthEnabled() ? 'grid-cols-2' : 'grid-cols-1'} gap-3`}>
                {/* Google Sign Up */}

  ...
  return (
    <div className="flex items-start justify-between">
      <div className="min-w-0 flex-1">
        ...
        {pilot && <p className="truncate text-sm text-gray-600">Pilot: {pilot}</p>}
        {model && <p className="truncate text-sm text-gray-500">Model: {model}</p>}
      </div>

      {/* Card image thumbnail */}
      <div className="ml-3 flex-shrink-0">
        <CardImage imageUrl={imageUrl} imageUrlSmall={imageUrlSmall} name={name} />
      </div>
    </div>
  );
};

...
export const FavoriteButton: React.FC<FavoriteButtonProps> = ({
  ...
  if (variant === 'icon') {
    return (
      <button
        ...
        disabled={isLoading}
        className={`transform text-2xl transition-colors duration-200 hover:scale-110 ${isFavorited ? 'text-red-500 hover:text-red-400' : 'text-muted-foreground hover:text-red-500'} ${isLoading ? 'cursor-not-allowed opacity-50' : 'cursor-pointer'} ${className} `}
        title={
          isFavorited
  ...
  return (
    <Button
      ...
      size={size}
      className={`flex items-center gap-2 ${isFavorited ? 'border-red-500 bg-red-500 hover:bg-red-600' : 'hover:border-red-300 hover:text-red-400'} ${className} `}
    >

...
async function getTrendsData(timeframe: 'day' | 'week' | 'month') {
  const trends = searchAnalytics.getSearchTrends(
    timeframe === 'day' ? 'hour' : timeframe === 'week' ? 'day' : 'week'
  );
...
async function getUserAnalytics(userId?: string) {
  ...
  return NextResponse.json({
    ...
    avgSearchesPerSession:
      Math.round(userBehavior.avgSearchesPerSession * 100) / 100,
    ...
    topFilters: Object.entries(userBehavior.preferredFilters)
      ...
      .map(([filter, count]) => ({
        filter,
        count,
        percentage: Math.round((count / userBehavior.totalSearches) * 100),
      })),
  });
}

async function getSearchSuggestions(userId?: string) {
  const suggestions = searchAnalytics.generateSearchSuggestions(
    userId || undefined
  );

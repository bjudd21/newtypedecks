...
export function useDecks() {
  ...
  const createDeck = useCallback(async (deckData: CreateDeckData): Promise<Deck | null> => {
    if (!isAuthenticated) {
  ...
  const updateDeck = useCallback(async (deckId: string, updateData: UpdateDeckData): Promise<Deck | null> => {
    if (!isAuthenticated) {
  ...
  const getUserDecks = useCallback(async (options?: {
    page?: number;
    limit?: number;
    search?: string;
    format?: string;

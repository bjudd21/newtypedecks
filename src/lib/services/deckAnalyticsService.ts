...
class DeckAnalyticsService {
  ...
  private calculateRarityDistribution(
    ...
    return Object.entries(rarityMap).reduce(
      (result, [rarity, count]) => {
        result[rarity] = {
          count,
          percentage: Math.round((count / totalCards) * 100 * 100) / 100,
  ...
  private calculateCostDistribution(
    ...
    return Object.entries(costMap).reduce(
      (result, [cost, count]) => {
  ...
  private calculateFactionDistribution(
    deckCards: DeckCard[]
  ): Record<string, { count: number; percentage: number }> {
    const totalCards = this.calculateTotalCards(deckCards);
    ...
    return Object.entries(factionMap).reduce(
      (result, [faction, count]) => {
        result[faction] = {
          count,
          percentage: Math.round((count / totalCards) * 100 * 100) / 100,
        };
  ...
  private generateSuggestions(deckCards: DeckCard[]): DeckSuggestion[] {
    ...
    if (totalCards < 40) {
      ...
      suggestions.push({
        type: 'remove',
        reason:
          'Deck size is too large, consider removing less impactful cards',
        priority: 'medium',
        impact: 0.6,
      });
    }
    ...
    if (lowCostPercentage < 25) {
      suggestions.push({
        type: 'add',
        reason:
          'Consider adding more low-cost cards for early game consistency',
        priority: 'medium',
        impact: 0.5,
      });
    }
